...
class Solver(nn.Module):
    ...
    def train(self, loaders):
        ...
        train_it, val_it = loaders[0], loaders[1]
        inputs_val = next(val_it)
        
        val_x_src, val_x_ref = torch.zeros_like(inputs_val['im_m']),torch.zeros_like(inputs_val['im_m'])
        val_y_src, val_y_ref = [],[]

        for i in range(args.val_batch_size):
            #print (i)
            if i<5:
                val_x_src[i] = inputs_val['im_m'][i]
                val_x_ref[i] = inputs_val['im_fe'][i]
                val_y_src.append(0)
                val_y_ref.append(1)
            elif i<10:
                val_x_src[i] = inputs_val['im_fe'][i]
                val_x_ref[i] = inputs_val['im_m'][i]
                val_y_src.append(1)
                val_y_ref.append(0)
            elif i<15:
                val_x_src[i] = inputs_val['im_m'][i]
                val_x_ref[i] = inputs_val['im_m2'][i]
                val_y_src.append(0)
                val_y_ref.append(0)
            else:
                val_x_src[i] = inputs_val['im_fe'][i]
                val_x_ref[i] = inputs_val['im_fe2'][i]
                val_y_src.append(1)
                val_y_ref.append(1)

        val_y_src = torch.tensor(val_y_src).cuda()
        val_y_ref = torch.tensor(val_y_ref).cuda()
        val_p_vec = inputs_val['att']
        ...
        for i in range(args.resume_iter, args.total_iters):
            ...
            inputs = next(train_it)
            
            if np.random.random()>0.2:
                if np.random.random()>0.5:
                    x_real, y_org = inputs['im_m'], torch.tensor(args.batch_size*[0]).cuda()
                    x_ref, x_ref2 = inputs['im_fe'], inputs['im_fe2']
                    y_trg = torch.tensor(args.batch_size*[1]).cuda()
                    
                else:
                    x_real, y_org = inputs['im_fe'], torch.tensor(args.batch_size*[1]).cuda()
                    x_ref, x_ref2 = inputs['im_m'], inputs['im_m2']
                    y_trg = torch.tensor(args.batch_size*[0]).cuda()
            else:
                if np.random.random()>0.5:
                    x_real, y_org = inputs['im_m'], torch.tensor(args.batch_size*[0]).cuda()
                    x_ref, x_ref2 = inputs['im_m2'], inputs['im_m']
                    y_trg = torch.tensor(args.batch_size*[0]).cuda()
                else:
                    x_real, y_org = inputs['im_fe'], torch.tensor(args.batch_size*[1]).cuda()
                    x_ref, x_ref2 = inputs['im_fe2'], inputs['im_fe']
                    y_trg = torch.tensor(args.batch_size*[1]).cuda()
            ...
            if (i+1) % args.print_every == 0:
                ...
                log = "Elapsed time [%s], Iteration [%i/%i], Model [%s], " % (elapsed, i+1, args.total_iters, args.exp_name)
                all_losses = dict()
            ...
            if (i+1) % args.sample_every == 0:
                ...
                utils.debug_image(nets_ema, args, val_x_src, val_x_ref, val_y_src, val_y_ref, val_p_vec, step=i+1)
