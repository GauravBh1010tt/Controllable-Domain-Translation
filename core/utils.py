...
@torch.no_grad()
def translate_and_reconstruct(nets, args, x_src, y_src, x_ref, y_ref, par_vec, filename):
    N, C, H, W = x_src.size()
    ...
    if args.par_dim>0:
        if args.par_mode == 'concat':
            s_ref = torch.cat([s_ref.float(), par_vec.float()], -1)
        else:
            masked_par_vec = nets.mask_mul(par_vec.float())
            s_ref = torch.cat([s_ref.float(), masked_par_vec.float()], -1)
    ...
    if args.par_dim>0:
        if args.par_mode == 'concat':
            s_src = torch.cat([s_src.float(), par_vec.float()], -1)
        else:
            masked_par_vec = nets.mask_mul(par_vec.float())
            s_src = torch.cat([s_src.float(), masked_par_vec.float()], -1)
...
@torch.no_grad()
def translate_using_latent(nets, args, x_src, y_trg_list, z_trg_list, psi, par_vec, filename):
    N, C, H, W = x_src.size()
...
@torch.no_grad()
def translate_using_reference(nets, args, x_src, x_ref, y_ref, par_vec, filename):
    N, C, H, W = x_src.size()
...
@torch.no_grad()
def debug_image(nets, args, x_src, x_ref, y_src, y_ref, p_vec, step, device='cuda'):
    x_src, y_src = x_src, y_src
    x_ref, y_ref = x_ref, y_ref

    N = x_src.size(0)

    # translate and reconstruct (reference-guided)
    filename = ospj(args.sample_dir, '%06d_cycle_consistency.jpg' % (step))
    translate_and_reconstruct(nets, args, x_src, y_src, x_ref, y_ref, p_vec, filename)
    ...
    if device=='cuda':
        z_trg_list = torch.randn(args.num_outs_per_domain, 1, args.latent_dim).repeat(1, N, 1).cuda()
    for psi in [0.5, 0.7, 1.0]:
        filename = ospj(args.sample_dir, '%06d_latent_psi_%.1f.jpg' % (step, psi))
        translate_using_latent(nets, args, x_src, y_trg_list, z_trg_list, psi, p_vec, filename)

    # reference-guided image synthesis
    filename = ospj(args.sample_dir, '%06d_reference.jpg' % (step))
    translate_using_reference(nets, args, x_src, x_ref, y_ref, p_vec, filename)
