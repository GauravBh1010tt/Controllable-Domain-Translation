...
from core.dataloader import HDF5DataLoader


@torch.no_grad()
def calculate_metrics(nets, args, step, mode, test_it=None):
    ...
    for trg_idx, trg_domain in enumerate(domains):
        ...
        #test_it = test_loader.create_iterator_n_iters(args.val_iters)
        ...
        for src_idx, src_domain in enumerate(src_domains):
            ...
            for i, inputs in enumerate(tqdm(test_it, total=len(test_it))):
                
                if src_domain == domains[0]:               
                    x_src, y_org = inputs[0], torch.tensor(args.batch_size*[0]).cuda()
                    x_ref, x_ref2 = inputs[2], inputs[3]
                    y_trg = torch.tensor(args.val_batch_size*[1]).cuda()
                    
                else:
                    x_src, y_org = inputs[2], torch.tensor(args.batch_size*[1]).cuda()
                    x_ref, x_ref2 = inputs[0], inputs[1]
                    y_trg = torch.tensor(args.val_batch_size*[0]).cuda()
                    
                par_vec = inputs[-1]
                N = args.val_batch_size
                
                #print (inputs[0].shape, inputs[1].shape, inputs[-1].shape)
                #bre
                ...
                for j in range(args.num_outs_per_domain):
                    ...
                    if args.par_dim>0:
                        if args.par_mode == 'concat':
                            #print (s_trg.shape, par_vec.shape)
...
def calculate_fid_for_all_tasks(args, domains, step, mode):
    ...
    for trg_domain in domains:
        ...
        for src_domain in src_domains:
            task = '%s2%s' % (src_domain, trg_domain)
            path_real = os.path.join(args.real_img_dir, trg_domain)
            path_fake = os.path.join(args.eval_dir, task)
